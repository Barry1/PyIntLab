...
class ScalarInterval:  # inheritance from object could be suppressed
    ...
    def __init__(
        ...
        if _orderguaranteed:
            ...
            self.lowerbound = float(min(float(x) for x in bounds))
            self.upperbound = float(max(float(x) for x in bounds))

    @staticmethod
    def _outward(lo: SupportsFloat, hi: SupportsFloat) -> tuple[float, float]:
        """Konservative Outward-Rundung: lo -> -inf, hi -> +inf."""
        return float(lo) if is_exact_float(lo) else math.nextafter(
            lo, -math.inf
